...
memory, which cannot be controlled by the user-supplied copier or 
user-supplied deleter.
...
`indirect_value` cannot be synthesized from `unique_ptr` as `unique_ptr` does not propagate `const` and has a deleted copy constructor.

`indirect_value` cannot be synthesized from `polymorphic_value` as `polymorphic_value`'s support for polymorphism
necessitates the use of virtual dispatch (type erasure) when producing copies - we do not want to pay for what we will not use.
